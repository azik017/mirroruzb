    ...
    def clone(self, link):
        ...
        try:
            ...
            if meta.get("mimeType") == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                msg += f'<b>Fayl Nomi: </b><code>{meta.get("name")}</code>\n<b>Hajmi: </b><code>{get_readable_file_size(self.transferred_size)}</code>'
                msg += '\n<b>Turi: </b><code>Folder</code>'
                msg += f'\n<b>Papkalar: </b><code>{self.total_folders}</code>ta'
                msg += f'\n<b>Fayllar: </b><code>{self.total_files}</code>ta'
                ...
                if INDEX_URL is not None:
                    ...
                    if SHORTENER is not None and SHORTENER_API is not None:
                        ...
                        buttons.buildbutton("⚡ Saytga Link", url)
            else:
                ...
                try:
                    ...
                    typ = 'File'
                try:
                    msg += f'\n<b>Hajmi: </b><code>{get_readable_file_size(int(meta.get("size")))}</code>'
        ...
        try:
            ...
            if drive_file['mimeType'] == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                try:
                    ...
                    msg += f'\n<b>Hajmi: </b><code>{get_readable_file_size(self.total_bytes)}</code>'
                    msg += f'\n<b>Turi: </b><code>{typee}</code>'
                    msg += f'\n<b>Fayllar: </b><code>{self.total_files}</code>'
                except TypeError:
                    pass
        except Exception as err:
            err = str(err).replace('>', '').replace('<', '')
